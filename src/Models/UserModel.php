...
usuario comparando su correo electrónico y contraseña con la base de datos. */
	final public static function login()
	{
		try {
			...
			$resultadoSqlPreparada = $conn->prepare($sql);

			var_dump($resultadoSqlPreparada);
			...
			sobre el objeto de declaración preparado `  $resultadoSqlPreparada` 
			...
			$resultadoSqlPreparada->execute($asignar_valores_parametros);
			// // var_dump($resultadoSqlPreparada->execute($asignar_valores_parametros));
			var_dump($resultadoSqlPreparada->rowCount());

			if ($resultadoSqlPreparada->rowCount() === 0) {
				return ResponseHttp::status400(CE_400 . ': UserModel-< Usuario no existe en base de datos >-');
			} else {
				foreach ($resultadoSqlPreparada as $datosSqlPreparada) {

					var_dump($datosSqlPreparada['password']);
					var_dump(self::getPassword());

					if (Security::validarMiPassword(self::getPassword(), $datosSqlPreparada['password'])) {
						...
						$IdentificadorToken = ['IDToken' => $datosSqlPreparada['IDToken']];
						...
						$dataLogin = [
							'name'  => $datosSqlPreparada['nombre'],
							'rol'   => $datosSqlPreparada['rol'],
							'token' => $token
						];

						var_dump($dataLogin);

						return ResponseHttp::status200($dataLogin);
						// exit;
					} else {
...
/* El código anterior es una función PHP llamada "postCrearUsuario" que se utiliza para crear un nuevo usuario en una base de datos. */
	final public static function postCrearUsuario()
	{
		...
		try {
			...
			$stresultSetql = "INSERT INTO usuario(nombre, dni, correo, rol, password, IDToken, fecha) VALUES (:nombre, :dni, :correo, :rol, :password, :IDToken, :fecha) ";

			$pStm = $conn->prepare($stresultSetql);
